...
# test11.py
#
# ABSOLUTE clock timing (faster times) - bad timing example
#
...
def main(mywin):
    ...
    lastLog = logging.LogFile('test11.log', level=logging.INFO, filemode='w')
    ...
    for i in range(len(fnames)*Nrep):
        ...
        # try to load an image during a short time period (bad idea)
        mypic[i] = visual.ImageStim(win=mywin, image=fnames[i], size=[.5, .5 * ar], contrast=1.0, ori=0)
